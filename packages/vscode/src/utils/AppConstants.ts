export default class AppConstants {
  ...
  public static PRICE_REGEX = new RegExp(/(?<=^\s*(?:[!Xx~-] )?(?:\([A-Z]\) )?(?:\d{4}-\d{2}-\d{2} ){0,3})[$£€]\d*[.,]?\d{1,2}(?= )/g); // TODO: Support additional currencies; possibly allow user configuration; possibly also support negative currencies with different style if it seems applicable
